...
document.addEventListener("DOMContentLoaded", function () {
    const phone_numberInput = document.querySelector("#phone_number");
    iti = window.intlTelInput(phone_numberInput, {
        initialCountry: "pt",
        utilsScript:
    ...
    function validateStep1() {
        ...
        if (!name || !universityId || !phone_number || !birthdate) {
            ...
            if (!iti.isValidNumber()) {
                displayError(
                    "You need to inform a valid number",
                    "phone_number"
                );
                valid = false;
            } else {
            ...
            if (inputDate > currentDate) {
                displayError(
                    "Birthdate cannot be greater than the current date",
                    "birthdate"
                );
                valid = false;
            } else {
...
image.addEventListener(
    ...
    function () {
        cropper = new Cropper(image, {
            ...
            viewMode: 3,
            minContainerWidth:
                image.width < 200 && image.height < 200 ? image.width * 2 : image.width,
            minContainerHeight:
...
function validatephone_numberInput(inputField) {
    inputField.value = inputField.value.replace(/[^0-9+ ]/g, "");
    const errorMessageBox = document.getElementById("error-message-phone_number");

    if (/[^0-9+ ]/.test(inputField.value)) {
        errorMessageBox.innerHTML =
            "<strong>Invalid input. Please enter only numbers, +, or .</strong>";
